...
export function PerfilScreen({ data }: { data: AlumnoPerfilData }) {
  if (!data.hasProfessor) {
    return <NoProfessorMessage label="Perfil" />;
  }

  const { studentName, assignedBy, routineName, memberSinceLabel, nextTrainingDayLabel } = data;
  ...
  return (
    <div className="mx-auto flex w-full max-w-sm flex-col gap-6 px-6 pb-28 pt-8">
      <div className="flex flex-col items-center gap-3 text-center">
        ...
        <p className="text-xl font-black text-ink">{studentName}</p>
      </div>
