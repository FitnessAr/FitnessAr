...
// Mensaje compacto reutilizado por cualquier pestaña del cliente que no tenga nada que mostrar
// sin profesor asignado (Rutina, Progreso) — la pantalla completa con los pasos a seguir vive en
// Home (features/cliente/home/no-professor-assigned.tsx).
export function NoProfessorMessage({ label }: { label: string }) {
  return (
    <div className="mx-auto flex w-full max-w-sm flex-col gap-2 px-6 pb-28 pt-8">
      ...
      <Link href="/cliente" className="mt-2 text-sm font-semibold text-brand">
        Volver a Inicio
      </Link>
    </div>
  );
